...
const QueryItem = ({ filtering,SetgOpen, gopen, setDataValue, dataValue, itm, value, size,classname="text-black" }) => {
                ...
                <ul id="dropdown-example" class="py-2 pl-3 space-y-2">
                    {
                        itm.columnName.map((nestItm) => {
                            ...
                            return <div className="pl-2 flex items-center w-full p-2 first-letter dark:text-white"><input type="checkbox" {...(dataValue[itm?.name]?.indexOf(nestItm.name) !== -1 && dataValue[itm?.name]?.indexOf(nestItm.parentName) !== undefined ? { checked: true } : { checked: false })} value={nestItm.index} onChange={((e) => {
                                onChangeData(itm?.name, e.target.checked, e.target.value, nestItm.name,"ind_" + itm.index)
                            })} /><span class={"text-" + sizeArr[size] + " pl-3 text-base font-normal transition duration-75 rounded-lg group dark:text-black "+classname}>{nestItm.name}</span></div>
                        })
                    }
                </ul> : <></>
        }
