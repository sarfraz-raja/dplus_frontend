...
const XAlertConfigure = () => {
    ...
    const openAdd = () => {
        setModalOpen(true);
        setModalBody(<XAlertConfigureForm setIsOpen={setModalOpen} resetting={true} formValue={{}} />);
    };

    const openEdit = (itm) => {
        setModalOpen(true);
        setModalBody(<XAlertConfigureForm setIsOpen={setModalOpen} resetting={false} formValue={itm} />);
    };
        ...
        <>
            <div className="flex flex-col h-[calc(100vh-4rem)] p-5 gap-4"
                ...
                <div className="flex items-center justify-between shrink-0">
                    ...
                    <span className="text-sm text-slate-500 font-medium">{total} alert{total !== 1 ? 's' : ''}</span>
                </div>
                ...
                <div className="flex items-center justify-between shrink-0 text-xs text-slate-500">
                    <span>
                        {total === 0 ? 'No alerts' : `Showing ${Math.min(startIdx + 1, total)}–${endIdx} of ${total}`}
                    </span>
                ...
                <div className="p-5">
                    <div className="flex items-center justify-between px-4 py-3 rounded-lg mb-5 shadow-sm" style={{ background: '#b91c1c' }}>
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-white font-semibold text-lg">Delete Alert</h2>
                        </div>
