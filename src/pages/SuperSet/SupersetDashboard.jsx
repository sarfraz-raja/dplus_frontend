    ...
    const loadDashboard = async () => {
      if (dashboardRef.current) {
        ...
        embedDashboard({
          ...
          id: finalUuid,
          supersetDomain: "http://192.168.0.100:8088", // No trailing slash!
          mountPoint: dashboardRef.current,
          fetchGuestToken: async () => {
            console.log("Fetching Guest Token...");
            
            // 1. LOGIN
            // const loginResp = await fetch("http://192.168.0.100:8088/api/v1/security/login", {
            //   method: 'POST',
            //   headers: { 'Content-Type': 'application/json' },
            //   body: JSON.stringify({
            //     username: "admin", password: "admin123", provider: "db", refresh: true
            ...
            // 2. GUEST TOKEN
            const guestResp = await fetch("http://192.168.0.100:8089/api/superset/guest-token", {
              method: 'POST',
              headers: {
                // 'Authorization': `Bearer ${access_token}`,
                'Content-Type': 'application/json'
              ...
              body: JSON.stringify({
                user: { username: "admin" }, // update to add logged in user info if available
