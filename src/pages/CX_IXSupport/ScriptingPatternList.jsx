...
const ScriptingPatternList = () => {
        ...
        return interdata.map((itm) => {
            let updateditm = {
                ...
                "logFile": <div className='flex justify-center'> <CstmButton classes='w-10' child={<Button icon={<UilFileDownload width={18}/>} onClick={() => {
                    ...
                    console.log(baseassetUrl + itm.script, "baseassetUrl+itm.fields.script")
                    dispatch(CommonActions.commondownload(baseassetUrl + itm.script, "log_file.zip"))
                }}></Button>} /></div>,
                "status": <div className='flex justify-center p-2 '><p className={statusColor[itm.status]}>{itm.status}</p></div>
            }
            return updateditm
        });
    })
    ...
    useEffect(() => {
        ...
        dispatch(cxixActions.get_cxix_scripting_form(uid))
    }, [])
    ...
    return <>
        ...
        <AdvancedTable
            ...
            headerButton={<><Button onClick={(e) => {
                ...
                setmodalBody(<ScriptingPatternForm uid={uid} isOpen={modalOpen} setIsOpen={setmodalOpen} resetting={true} formValue={{}} />)
            }}
...
export default ScriptingPatternList;
